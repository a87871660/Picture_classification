...
def deep_CNN(im_x, batch_size, n_classes):
    ...
    with tf.variable_scope('conv2') as scope:
        w_conv2 = tf.Variable(weight_variable([3, 3, 64, 32], 1.0), name='weights', dtype=tf.float32)
    ...
    with tf.variable_scope('conv_3') as scope:
        w_conv3 = tf.Variable(weight_variable([3, 3, 32, 16], 1.0), name='weights', dtype=tf.float32)
    ...
    with tf.variable_scope('local4') as scope:
        ...
        h_fc2 = tf.nn.relu(tf.matmul(h_fc1, w_fc2)+b_fc2, name='local4')

    h_fc2_dropout = tf.nn.dropout(h_fc2, 0.5, name='dropout')
...
def trainning(loss, learning_rate):
    with tf.name_scope('optimizer'):
        ...
        learning_rate = tf.train.exponential_decay(learning_rate, global_step, decay_steps=5000, decay_rate=0.9, staircase=True, name='learning_rate')
